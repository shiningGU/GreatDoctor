...
#import "shfz_AFNetworking.h"

@implementation shfz_AFNetworking
//检测网络状态
+ (void)netWorkStatus
{
    /**
    ...
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == -1) {
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"友情提示^ ^" message:@"当前为未知网络!" delegate:self cancelButtonTitle:nil otherButtonTitles:@"检查网络连接", nil];
            [alertView show];
        }else if (status == 0){
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"友情提示^ ^" message:@"当前没有网络!" delegate:self cancelButtonTitle:nil otherButtonTitles:@"检查网络连接", nil];
            [alertView show];
        }
    }];
}
