...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView == _alert3) {
        if ([self.getWay isEqualToString:@"newphone"]) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"notice", nil) message:@"绑定成功" delegate:self cancelButtonTitle:NSLocalizedString(@"sure", nil) otherButtonTitles: nil];
            _alert4 = alert;
            [alert show];
        }else {
            [self dismissViewControllerAnimated:YES completion:^{
...
- (void)viewDidLoad
{
    ...
    label.frame = CGRectMake(15, 73, self.view.frame.size.width - 30, 21);
    ...
    _telLabel.frame=CGRectMake(15, 102, self.view.frame.size.width - 30, 21);
    ...
    _verifyCodeField.frame = CGRectMake(15, 131, self.view.frame.size.width - 30, 46);
    ...
    _timeLabel.frame = CGRectMake(15, 189, self.view.frame.size.width - 30, 40);
    ...
    _repeatSMSBtn.frame = CGRectMake(15, 189, self.view.frame.size.width - 30, 30);
    ...
    if ([self.getWay isEqualToString:@"newphone"]) {
        [_submitBtn setTitle:@"绑定" forState:UIControlStateNormal];
    }else{
        [_submitBtn setTitle:@"解绑" forState:UIControlStateNormal];
    }
    _submitBtn.backgroundColor = RGBACOLOR(0, 178, 255, 1.0);
    _submitBtn.frame = CGRectMake(15, 240, self.view.frame.size.width - 30, 42);
    [_submitBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_submitBtn addTarget:self action:@selector(submit) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_submitBtn];
    
    _voiceCallMsgLabel = [[UILabel alloc] init];
    _voiceCallMsgLabel.frame = CGRectMake(15, 288, self.view.frame.size.width - 30, 25);
    ...
    _voiceCallButton.backgroundColor = RGBACOLOR(0, 178, 255, 1.0);
    _voiceCallButton.frame = CGRectMake(15, 320, self.view.frame.size.width - 30, 42);
